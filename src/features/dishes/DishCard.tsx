...
import { GetImageById } from '../../hooks/imageHooks';
import { useAppDispatch } from '../../store/store';
import { fetchImageById } from '../images/imageApi';
...
function DishCard(props: DishCardProps) {
    ...
    useEffect(() => {
        appDispatch(fetchImageById(props.dish.id!));
    }, [appDispatch, props.dish.id]);
    const image = GetImageById(props.dish.id!);


    return (
        <div className="sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 p-4 shadow-md">
            <img className="sm:h-48 md:h-56 lg:h-64 xl:h-72 w-full object-cover rounded-lg" src={image?.url} alt={props.dish.name} />
